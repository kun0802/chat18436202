...
public class ClientLogin extends JFrame implements ActionListener{//类名：ClientLogin  模板：对象的模板（一个类有很多个对象）
	...
	public ClientLogin(){//构造方法省略（系统会默认生成一个方法）
        ...
        this.setLocationRelativeTo(null);//将窗口居中显示，可有可无
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==jb1){
			...
			//创建USer对象
			User user=new User();//对象存储堆内存，引用变量存储在栈内存
